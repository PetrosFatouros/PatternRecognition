    ...
    for features in features_list:
        ...
        for train_index, test_index in K_fold.split(features, labels):
            ...
            for i in range(3):  # one vs all (home, draw, away)
                # Train classifier
                weights = train_ls(x_train, y_train, i)

                # Evaluate classifier
                train_accuracy = test_ls(x_train, y_train, weights, i)
                train_accuracy_list.append(train_accuracy)
                test_accuracy = test_ls(x_test, y_test, weights, i)
...
def test_ls(features, labels, w, code):
